...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnRegister.setOnClickListener(v -> {
            ...
            if (TextUtils.isEmpty(name)) { edtName.setError("Vui lòng nhập họ tên"); return; }
            if (!Utils.isValidUsername(username)) { edtUsername.setError("Username 3-20 ký tự, chữ/số/._, bắt đầu bằng chữ"); return; }
            if (!Utils.isValidEmail(email)) { edtEmail.setError("Email không hợp lệ"); return; }
            if (!Utils.isStrongPassword(password)) { edtPassword.setError("Mật khẩu tối thiểu 6 ký tự, gồm chữ & số"); return; }
            if (!password.equals(password2)) { edtPassword2.setError("Mật khẩu không khớp"); return; }

            Role role = rbTeacher.isChecked() ? Role.TEACHER : Role.STUDENT;

            FakeApiService.ApiResult<User> result =
                    FakeApiService.getInstance().register(name, username, email, password, role);
